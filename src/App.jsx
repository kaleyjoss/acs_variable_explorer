...
function NavBtn({ active, onClick, label, color="#1e3a5f" }) {
  return <button onClick={onClick} style={{ background:active?color:"white", color:active?"white":color, border:`1.5px solid ${color}`, borderRadius:6, padding:"4px 12px", cursor:"pointer", fontSize:13, fontWeight:600 }}>{label}</button>;
}
...
function ChildList({ children, onSelect, headerLabel, getBestId }) {
  ...
  return (
    <div style={{ background:"white", borderRadius:10, border:"1.5px solid #cbd5e1", overflow:"hidden", marginBottom:14 }}>
      ...
      {keys.map((key,i) => {
        const bestId=getBestId(key), hasKids=Object.keys(children[key].__children||{}).length>0;
        return (
    ...
    <div style={{ fontFamily:"system-ui, sans-serif", maxWidth:860, margin:"0 auto", padding:24, background:"#f8fafc", minHeight:"100vh" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div>
          ...
          <p style={{margin:"3px 0 0", fontSize:10, alignItems:"left", color:"#1e3a5f" }}>This is a webapp to navigate the data within the American Community Survey (ACS). This webpage allows you to navigate through topics and subtopics, while displaying the relevant variable ID from the ACS that you'll need to use for calling the API. It also generates an R script for using the package tidycensus to pull the data you're navigated to-- in order to specify which data you want, use the dropdowns to specify geography and year (state is optional).</p>
          <p style={{margin:"3px 0 0", fontSize:10, alignItems:"left", color:"#1e3a5f" }}>The variables I collected from the ACS API webpages for the 1-year data. These can be found on api.census.gov.../variables.html (for example: https://api.census.gov/data/2022/acs/acs5/profile/variables.html) If want to add to this site, you find any bugs or you want to request features, the whole site is deployed from https://github.com/kaleyjoss/acs_variable_explorer/. </p>
      ...
      {showUpload && (
        <div style={{ background:"white", border:"1.5px solid #cbd5e1", borderRadius:10, padding:16, marginBottom:20 }}>
          <p style={{ margin:"0 0 8px", fontSize:13, color:"#475569" }}>
            Upload <strong>1yr_clean_varnames_shortened.csv</strong> or any CSV with <strong>id</strong>, <strong>label_clean</strong>, <strong>detail</strong>, <strong>label_varname</strong>, <strong>detail_varname</strong> columns.
          </p>
